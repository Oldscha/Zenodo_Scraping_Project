...
To retrieve data from the Zenodo repository, several R packages are used, including "httr", "tidyverse", and "zen4R". The strategy included the building of the REST API endpoint. So the first step involve creating an instance of the Zenodo manager to interact with the Zenodo API and perform desired actions. In order to search for publications relevant to and belonging to the social science field from the large number of publications, the keyword search is considered. But because keywords are not entered reliably by Zenodo users and Zenodo did not offer its own function to search for communities, "zen4R" package is used.
